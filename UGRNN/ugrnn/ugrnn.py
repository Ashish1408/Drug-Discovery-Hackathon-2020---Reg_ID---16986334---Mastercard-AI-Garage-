...
class UGRNN(object):
    def __init__(self, model_name, encoding_nn_hidden_size, encoding_nn_output_size,
        ...
        self.initializer_fun = nn_utils.get_initializer('random')#(config.initializer)
        ...
        hidden1, prediction_op, g_structue = self.add_prediction_op(self.local_input_pls[0],
                                                           self.path_pls[0],
                                                           self.logp_pls[0],
        ...
        for i in xrange(1, self.batch_size):
            with tf.control_dependencies([self.prediction_ops[i - 1]]):
                hidden1, prediction_op, g_structue = self.add_prediction_op(self.local_input_pls[i],
                                                                            self.path_pls[i],
                                                                            self.logp_pls[i],
    ...
    def create_UGRNN_variable(self):
        with tf.variable_scope(self._Unique_ID+"EncodingNN") as scope:
            contextual_features = tf.get_variable("contextual_features",
                                                  [config.max_seq_len * config.max_seq_len * 4, self.encoding_nn_output_size],
                                                  dtype=tf.float32,
                                                  initializer=tf.constant_initializer(0),
                                                  trainable=False)

            with tf.variable_scope('hidden1') as scope:
                weights = nn_utils.weight_variable([self.encoding_nn_input_size, self.encoding_nn_hidden_size], initializer=self.initializer_fun)
            ...
            with tf.variable_scope('output') as scope:
                weights = nn_utils.weight_variable([self.encoding_nn_hidden_size, self.encoding_nn_output_size],  initializer=self.initializer_fun)
        ...
        with tf.variable_scope(self._Unique_ID+"OutputNN") as scope:
            with tf.variable_scope('hidden1') as scope:
                weights = nn_utils.weight_variable([self.output_nn_input_size, self.output_nn_hidden_size], self.initializer_fun, 'weights_decay')
                biases = nn_utils.bias_variable([self.output_nn_hidden_size])
                self.trainable_variables.append(weights)
                self.trainable_variables.append(biases)
                
            with tf.variable_scope('output') as scope:
                weights = nn_utils.weight_variable([self.output_nn_hidden_size, 1], self.initializer_fun)
                biases = nn_utils.bias_variable([self.output_nn_hidden_size])
                self.trainable_variables.append(weights)
                self.trainable_variables.append(biases)
    ...
    def add_prediction_op(self, feature_pl, path_pl, logp_pl, sequence_len, flattened_idx_offset):
        with tf.variable_scope(self._Unique_ID+"EncodingNN", reuse=True) as scope:
            step = tf.constant(0)
            contextual_features = tf.get_variable("contextual_features")
            contextual_features = contextual_features.assign(tf.zeros([config.max_seq_len * config.max_seq_len * 4,
            ...
            indices = tf.stack([tf.range(0, sequence_len), tf.range(0, sequence_len)], axis=1)
            step_feature = tf.gather_nd(feature_pl, indices)

            inputs = tf.concat([step_contextual_features, step_feature], 1)
            ...
            if self.add_logp:
                outputNN_input = tf.concat([x, molecule_encoding], 1)
            else:
                outputNN_input = molecule_encoding
                
        with tf.variable_scope(self._Unique_ID+"OutputNN", reuse=True) as scope:
            hidden1, prediction_op = UGRNN.apply_OutputNN(outputNN_input, config.activation_function)

        return hidden1, prediction_op, molecule_encoding
...
#    @staticmethod
    def body(self,sequence_len, step, feature_pl, path_pl, flattened_idx_offset,
        ...
        zero = tf.constant(0)
        one = tf.constant(1)
        input_begin = tf.stack([zero, step, zero])

        input_idx = tf.slice(path_pl, input_begin, [-1, 1, 1])
        input_idx = tf.reshape(input_idx, [-1])

        indices = tf.stack([tf.range(0, sequence_len), input_idx], axis=1)
        step_feature = tf.gather_nd(feature_pl, indices)

        output_begin = tf.stack([zero, step, one])
        ...
        nn_inputs = tf.concat([step_contextual_features, step_feature],1)
    ...
    def add_training_ops(self):
        ...
        loss_op = self.loss_op + config.weight_decay_factor * tf.add_n([tf.nn.l2_loss(v) for v in tf.get_collection('weights_decay')])
    ...
    def add_loss_op(self):
        loss_op = [
                    tf.square(tf.subtract(self.prediction_ops[i], self.target_pls[i])) 
...
#        logger.info('Start Training')
        ...
        for epoch in xrange(0, epochs):
            ...
            train_dataset.reset_epoch(permute=False)
 ...
 #                plt.pause(0.05)
                 logger.info(
                     "Epoch: {:}, LR {:.8f}  Train CE: {:.4f}, Train AUC: {:.4f} Val CE: {:.4f}, Val AUC {:.4f}".
                         format(epoch, learning_rate[0], train_metric[0], train_metric[2], validation_metric[0],  validation_metric[2],
                                precision=5))
...
#        save_results(train_results_file_path, train_dataset.labels, self.predict(sess, train_dataset))
    ...
    def Hidden(self, sess, dataset):
#        print(dataset)
        dataset.reset_epoch()
        hidden = np.empty((dataset.num_examples, self.output_nn_hidden_size))
        for i in xrange(0, dataset.num_examples):
            feed_dict = self.fill_feed_dict(dataset, 1)
            hidden_value = sess.run([self.hidden1[0]], feed_dict=feed_dict)
            hidden[i,:] = hidden_value[0].ravel()
        return hidden
...
#            print(g_structure)
            g_structures[i,:] = g_structure[0].ravel()
        return g_structures
...
#'''________________________________________________________________________________'''ORIGINAL
...
#        saver.save(sess, save_path=checkpoint_file)
    ...
    def get_contextual_feature(contextual_features, index,
        ...
        indices = tf.stack(values, axis=1, name='pack')
    ...
    def apply_EncodingNN(inputs, activation_type):
        ...
        with tf.variable_scope('output') as scope:
            ...
            encodes = activation_fun(tf.matmul(hidden1, weights) + biases)
    ...
    def apply_OutputNN(inputs, activation_type):
        ...
        with tf.variable_scope('output') as scope:
            ...
            outputs = activation_fun(tf.matmul(hidden1, weights) + biases)
            
            return hidden1, outputs