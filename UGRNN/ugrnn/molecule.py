...
#            self.graph.add_node(i, attr_dict={"atom_features": Molecule.atom_features(atom)})
            self.graph.add_node(i, atom_features=Molecule.atom_features(atom))
            for neighbour in atom.GetNeighbors():
                neighbour_idx = neighbour.GetIdx()
                bond = m.GetBondBetweenAtoms(i, neighbour_idx)
#                self.graph.add_edge(i, neighbour_idx,attr_dict={"bond_features": Molecule.bond_features(bond)})
                self.graph.add_edge(i, neighbour_idx,bond_features=Molecule.bond_features(bond))
    ...
    def create_directed_graphs(self):
        ...
        # as the sink node
        path = nx.single_source_dijkstra_path(self.graph, 0)
...
#            print(self,len(path),self.no_of_atoms)
            G = nx.DiGraph()
...
#                print(node, list(nx.edges(G, node)))
                edge = list(nx.edges(G, node))[0]
#                print(edge)
                if edge[1] in list(no_of_incoming_edges):
    ...
    def create_feature_vectors(self):
        ...
        self.local_input_vector = np.zeros((self.no_of_atoms, self.no_of_atoms, Molecule.num_of_features()))
        

        for idx in xrange(self.no_of_atoms):
            sorted_path = self.directed_graphs[idx, :, :]
            path = nx.shortest_path(self.graph, source=idx)
...
#                    self.graph.add_edge(node1, cycle_name, attr_dict={"bond_features": Molecule.bond_features_between_contract_rings()})
                    self.graph.add_edge(node1, cycle_name, bond_features = Molecule.bond_features_between_contract_rings())
                    continue
...
#                    self.graph.add_edge(neighbour, cycle_name, attr_dict={"bond_features": edge_attrs})
                    self.graph.add_edge(neighbour, cycle_name, bond_features= edge_attrs)
                    self.graph.remove_edge(node1, neighbour)
#            print(cycle_name, Molecule.atom_features_of_contract_rings(0))
            nx.set_node_attributes(self.graph,name= "atom_features", values={cycle_name: list(Molecule.atom_features_of_contract_rings(0))})
...
#        print('Entering into atom feature')
#        np.array(
#                Molecule.one_of_k_encoding_unk(atom.GetSymbol(),
#                                                       ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl',
#                                                        'Br', 'Mg', 'Na',
#                                                        'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K',
#                                                        'Tl', 'Yb',
...
#        )
    def atom_features(atom):
        x = np.array(list(Molecule.one_of_k_encoding_unk(atom.GetSymbol(),
                                                       ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl',
                                                        'Br', 'Mg', 'Na',
                                                        'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K',
                                                        'Tl', 'Yb',
                                                        ...
                                                        'Mn',
                                                        'Zr', 'Cr', 'Pt', 'Hg', 'Pb', 'Unknown'])) +
                        list(Molecule.one_of_k_encoding(atom.GetDegree(), [0, 1, 2, 3, 4, 5])) +
                        list(Molecule.one_of_k_encoding_unk(atom.GetTotalNumHs(), [0, 1, 2, 3, 4])) +
                        list(Molecule.one_of_k_encoding_unk(atom.GetImplicitValence(), [0, 1, 2, 3, 4, 5])) +
                        list([atom.GetIsAromatic()]))
#        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~',x)
        return x
    ...
    def atom_features_of_contract_rings(degree):
        return np.array(list(Molecule.one_of_k_encoding_unk('Unknown',
                                                       ['C', 'N', 'O', 'S', 'F', 'Si', 'P', 'Cl',
                                                        'Br', 'Mg', 'Na',
                                                        'Ca', 'Fe', 'As', 'Al', 'I', 'B', 'V', 'K',
                                                        'Tl', 'Yb',
                                                        ...
                                                        'Mn', 'Zr',
                                                        'Cr', 'Pt', 'Hg', 'Pb', 'Unknown'])) +
                        list(Molecule.one_of_k_encoding(degree, [0, 1, 2, 3, 4, 5])) +
                        list(Molecule.one_of_k_encoding_unk(0, [0, 1, 2, 3, 4])) +
                        list(Molecule.one_of_k_encoding_unk(0, [0, 1, 2, 3, 4, 5])) +
                        list([0]))
