...
def get_prediction_from_model(model_name, encoding_nn_hidden_size, encoding_nn_output_size,
    ...
    with tf.Graph().as_default():
        ...
        sess = tf.Session() #config=config,'from_detection_checkpoint':true
...
def main(_):
    ...
    for i in xrange(0, len(FLAGS.model_names)):
        predictions = get_prediction_from_model(
                                                FLAGS.model_names[i], 
    ...
    predictor_names = [ "Random forest classification", "Top 10", "Greedy"]
    ...
    for fun, name in zip(ensemble_predictor, predictor_names):
        ...
        logger.info("Method {:} CE: {:}, In-Ac: {:}, F-1: {:}".format(name, prediction_metric[0], prediction_metric[1],
                                                                   prediction_metric[2]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_names', nargs='+', type=str, default=list(["default_model"]),
                        help='Name of the models used for prediction')
  
    parser.add_argument('--model_params', default = list([[12,3,5]]), 
                        help="Model Parameters", dest="model_params", type=model_params, nargs='+')
    
    parser.add_argument('--output_dir', type=str, default= './data/DILI/Model_Train_DILI_Data',
                        help='Root Directory where the  model parameters are stored')

    parser.add_argument('--test_file', type=str, default='./data/DILI/test_DILI.csv',
                        help='Path to the csv file containing test data set')

    parser.add_argument('--validation_file', type=str, default='./data/DILI/validate_DILI.csv',
    ...
    parser.add_argument('--add_logp', dest='add_logp', action='store_true')
    parser.set_defaults(add_logp=False)

    parser.add_argument('--optimize_ensemble', dest='optimize_ensemble', action='store_true')
